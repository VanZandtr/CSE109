...
private:
	...
	char* unpack(unsigned int packet) const;
};
...
bool Hash_t::remove(char data)
{
	...
	if(find(data))
	{
		for(size_t i = 0; i< buckets[index].size(); i++)
		{
			char* array = unpack(buckets[index][i]);
 ...
 //retval += return string
	...
	for(size_t i = 0; i<buckets.size(); i++)
	{
		...
		else
		{
			for(size_t j = 0; j<buckets[i].size(); j++)
			{
				unsigned int temp = buckets[i][j];
				char* array = unpack(temp);
...
char* Hash_t::unpack(unsigned int packet) const
{
